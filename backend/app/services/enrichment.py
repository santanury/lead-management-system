...
class EnrichmentService:
    ...
    Uses Gemini to "hallucinate" (retrieve knowledge of) company details asynchronously.
    """

    async def enrich_lead(self, company_name: str, email: str) -> EnrichmentData:
        """
        Enriches lead data with company information and email validation asynchronously.
        """
        is_valid_email = self._validate_email(email)
        company_info = await self._enrich_company_info(company_name, email) if is_valid_email else None
    ...
    async def _enrich_company_info(self, company_name: str, email: str) -> dict:
        """
        Uses Gemini to get company details asynchronously.
        """
        ...
        try:
            ...
            # Await the async gemini_client call
            result = await gemini_client.generate_json_response(prompt, use_search=True)
